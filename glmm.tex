...
Mixed effects modeling -- alternativeley called \textit{hierarchical} or \textit{multilevel modeling} is a straightforward extension of (generalized) linear modeling as discussed in the previous chapter.
...
In corpus linguistics, there could be clusters of observations defined by individual speakers, registers, genres, modes, lemmas occurring in constructions, etc.
...
Then, Section~\ref{sec:modelspecificationandmodelingassumptions} provide a moderately technical introduction to the important technicalities in mixed modeling, including a discussion of when a factor should be included as a random effect and when as a fixed effect.
Section~\ref{sec:specifyingmodelsusinglme4inr} then shows how mixed models are specified and interpreted using R.
...
However, even when a data set has a nested structure by accident, standard packages in R will treat them as nested, and a closer look at data sets should be part of any protocol for using GLMMs in corpus studies (see Section~\ref{sec:specifyingmodelsusinglme4inr}).
...
This is achieved, in fact, by specifying a second (linear) model which predicts the group-level random effect itself, and the second-level predictor is a fixed effect in this model.
...
Before moving on to the more technical discussion of hierarchical model specification in Section~\ref{sec:modelspecificationandmodelingassumptions}, one more basic concept will be discussed in this section, namely the data patterns that gives rise to \textit{varying intercepts} and \textit{varying slopes}. 
...
At the same time, the estimation of coefficients for a (dummy-coded) fixed effect becomes less precise and feasible with large numbers of levels, and at a certain point, a random effect might be the best option for technical reasons.
...
The conditional modes are \textit{shrunken} towards the overall intercept as a result (\textit{shrinkage}), and they tend to be more leveled compared to fixed effects estimates.
...
$x_1$ was a binary variable (such as singular\slash plural) and $x_2$ a continuous variable (such as NP length).
...
In this chapter, a more technical approach was therefore suggested; see Section~\ref{sec:choosingbetweenrandomandfixedeffects}.
...
An alternative option are parametric bootstrap replacements for the LR test (see Section~\ref{sec:specifyingmodelsusinglme4inr}).
...
This section shows briefly how more complex models are specified before Section~\ref{sec:specifyingmodelsusinglme4inr} demonstrated how models are estimated in R, including interpretations of the output.
...
Importantly, the number of variance parameters to be estimated obviously increases with more complex model specifications, and the estimation of the parameters in the presence of complex variance-covariance matrices requires considerably more data than estimating a single variance parameter.
...
In Section~\ref{sec:interpretingtheoutputoflme4}, it will be shown how to interpret the lme4 output appropriately.
...
The lme4 syntax treats second-level predictors like interactions between a fixed and a random effect, see also Section~\ref{sec:specifyingmodelsusinglme4inr}.
...
It is thus recommended to include the speaker grouping factor, either as a fixed effect (up to five speakers) or random effect (more speakers).
Second, there might be time parameters such as years of learning.
Third, the errors might be correlated.

We now assume we have a sample from a learner corpus of German as a second language.
...
Additionally, we add a term for the number of years the learners have learned German (potentially logarithmized or otherwise transformed).
...
Since we have a random intercept and a random slope, the second-level model becomes more complex, and we need to distinguish between $\gamma_f^{\alpha}\cdot u_f^j$ for the random intercept and $\gamma_f^{\beta}\cdot u_f^j$ for the random slope.
...
This is already a quite complex model, but most likely not yet adequate.
Since the performance of learners after $n+1$ years of learning is usually correlated to a high degree with their performance after $n$ years.
This will lead to \textit{autocorrelation} in the errors, violating basic assumptions.
To take care of this, model which allow for explicitly specified error structures must be used.
While these are well-understood and implemented in R packages for linear models (see \citealt{Fox2016} and \citealt{ZuurEa2009}), there exists no simple solution for GLM(M)s.
Anyone wishing to use such models will have to consult original literature.

\subsection{Specifying model using lme4 in R}
...
Only the formula is given from now on since the rest of the function call remains the same.
...
When there is more than one random effect, several bracketed terms can simply be added.
...
\subsection{Interpreting the output of lme4}
\label{sec:interpretingtheoutputoflme4}
...
Since the bootstrap (especially with smaller original sample sizes) tends to run into replications where the estimation of the variance fails and is thus output as $0$, the bootstrap interval is skewed to the left, while the profile confidence interval frames the true value symmetrically.
The bootstrap is thus not always more robust or intrinsically better.

Although the authors of the lme4 package advise against it, a significance test on the deviances of a simple GLMM and a GLMM with an added single random effect can be performed with the \texttt{anova} function.
...
\begin{boxedminipage}{\textwidth}
  ...
  The authors state on p.~399 that they collapsed all head noun lemmas with less than four occurrences into one category.
  They do not give any reason except that otherwise ``difficulties'' would arise, but it is the advantage of random effects modeling that it can deal with a situation where categories have low numbers of observations (see \textit{shrinkage} in Section~\ref{sec:choosingbetweenrandomandfixedeffects}).
