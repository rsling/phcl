...
\subsection{Choosing between random and fixed effects}
...
From the estimated variance and the data, the estimator then predicts the \textit{conditional modes} in GLMMs (\textit{conditional means} in LMMs) for each group (see \citealt{Bates2010}), which is the numerical value which software packages like lme4 peoduce, and these values are sometimes sloppily called ``random effects'' by practitioners.
...
If, however, the number of groups is large enough, the next thing to consider is the number of observations per group.
...
This section finished with a comparison with an illustration.%
...
For this, 1,000 data sets were simulated (see Chapter~XYZ) which corresponded ideally to the model in (\ref{eq:glmm03}) and (\ref{eq:glmm04}).
...
Even with as few as five levels of a grouping factor, random and fixed effects lead to similar results, with different advantages and disadvantages.
...
\subsection{Significance testing, model selection and coefficients of determination}
...
\citet[245--247]{GelmanHill2006} summarise the diverging recommendations and their various justifications, concluding that there is essentially no clean conceptual way of deciding what should be a random effect and what a fixed effect.
...
This can be achieved by likelihood ration testing comparing a model including the random effect and a model not including it.
Such models are called \textit{nested} (not to be confused with nested effects discussed in Section~\ref{sec:crossedandnestedeffects}).
A much more robust option uses a parametric bootstrap (see Section~\ref{sec:usinglme4}).
...
Also, coefficients of determination ($R^2$) should not be used for model selection, because they usually do not penalize complexity enough (\ie the coefficients rise with added complexity).
...
Using the simulations described in the last section, Figures~\ref{fig:rsqglmmj5i20} and \ref{fig:rsqglmmj5i20} show that the marginal $R^2$ is roughly the same as Nagelkerke's $R^2$ in a GLM which ignores the grouping, and the conditional is roughly the same as Nagelkerke's $R^2$ in a GLM which includes the grouping factor as a fixed effect, given that all the estimator converges for all these three types of models.

\begin{figure}[!htpb]
  \centering
  \includegraphics[width=0.6\textwidth]{graphics/rsqglmmj5i20}
  \caption{Distribution of Nakagawa \& Schielzeth's $R^2$ in the simulations described in Section~\ref{sec:differencesbetweenrandomandfixedeffects}}
...
\begin{figure}[!htpb]
  ...
  \caption{Nagelkerke's $R^2$ in the simulations described in Section~\ref{sec:differencesbetweenrandomandfixedeffects} for a GLM that ignores the grouping factor and a model that includes it as a fixed effect}
