...
Section~\ref{sec:specifyingmodelsusinglme4inr} then shows how mixed models are specified using tze \texttt{lme4} package in R, and Section~\ref{sec:afterfittingmodelswithlme4} deals with the interpretation of the output.
...
If the nested (simpler) model still contains a random effect, the \texttt{update} function can be used to build the simpler model, and in addition to the \texttt{anova} command, there is a drop-in replacement for LR tests using bootstrap methods in the \texttt{PBmodcomp} package.
...
The coefficient of determination (pseudo-$R^2$) according to \citet{NakagawaSchielzeth2013} can be computed using the function \texttt{r.squaredGLMM} from the \texttt{MuMIn} package.
