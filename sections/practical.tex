\subsection{Specifying models using \texttt{lme4} in R}
...
See Section~\ref{sec:morecomplexmodels} for an example.
...
\begin{table}
  ...
  \resizebox{\textwidth}{!}{
    ...
    Model           &                   &                        & Factor       &             &        &         \\
    level           & Regressor         & $\text{p}_{\text{PB}}$ & level        & Coefficient & CI low & CI high \\
    \midrule
...
\begin{quote}
  ...
  The sample size was \textit{n=}5,063 with 1,134 cases with the genitive and 3,929 cases with case identity.
  ...
  I ran a parametric bootstrap (using the \mbox{\textit{confint.merMod}} function from \textit{lme4}) with 1,000 replications and using the percentile method.
  The resulting 95\% bootstrap confidence intervals are reported in Table~\ref{tab:bigtable} in columns \textit{CI low} and \textit{CI high}.
  ...
  A drop-in bootstrap replacement for the Likelihood Ratio Test in the form of the function \textit{PBmodcomp} from the \textit{pbkrtest} package \citep{HalekohHojsgaard2014} was used. 
  ...
  Of all predictors, only \textit{Kindfreq} ($p_{\text{PB}}=0.095$) could be seen as slightly too high to be convincing, failing at sig=0.05.
\end{quote}
